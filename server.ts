...
app.delete("/api/tasks/:taskId", (req, res) => {
  ...
  // Physically delete physical files associated with the deleted task before removing from state
  const filesToDelete = taskFiles.filter(f => matchTask(f.taskId));
  for (const file of filesToDelete) {
    const fullPath = path.join(getEnterpriseDir(), file.path);
    try {
      if (fs.existsSync(fullPath)) {
        fs.unlinkSync(fullPath);
        console.log(`Deleted physical file associated with deleted task: ${fullPath}`);
      }
    } catch (err) {
      console.error(`Failed to delete physical file ${fullPath} on task deletion:`, err);
    }
  }
